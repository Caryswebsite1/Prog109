...
function ValidateForm() {
    ...
    // arguments are: id of form input item, id of that items associated error item, max char, 
    // Not allowed chartype, required char types, required, is this an array value from a selection input...

    bFirstName = GenericFormValidator('FirstName', 'FirstNameError', 20, lettersOnly, null, true, false);
    bLastName = GenericFormValidator('LastName', 'LastNameError', 50, lettersOnly, null, true, false);
    bEmail = GenericFormValidator('Email', 'EmailError', 100, null, emailChars, true, false);
    bPhone = GenericFormValidator('Phone', 'PhoneError', 15, NumsOnly, null, true, false);
    bUserName = GenericFormValidator('UserName', 'UserNameError', 12, null, null, true, false);
    bPassword = GenericFormValidator('Password', 'PasswordError', 7, null, null, true, false);
    bStreetAdd = GenericFormValidator('StreetAddress', 'StreetAddError', 100, null, null, true, false);
    bCity = GenericFormValidator('City', 'CityError', 100, null, null, true, false);
    bState = GenericFormValidator('State', 'StateError', null, null, null, true, true);
    bCountry = GenericFormValidator('Country', 'CountryError', null, null, null, true, true);

    // have to check country before sending zip.
    if (bCountry && document.getElementById("Country").value === "USA") {
        // zip is required.
        bZip = GenericFormValidator('Zip', 'ZipError', 5, null, null, true, false);
    }
    else {
        // zip not required..
        bZip = GenericFormValidator('Zip', 'ZipError', 5, null, null, false, false);
    }
...
// ----------------------------------------------------------------------------------
function GenericFormValidator(itemID, itemErrorID, MaxLength, NotAllowedCharType, ReqCharTypes, bRequired, bArray) {
    ...
    if (bArray) {
        if (formItem.options[formItem.selectedIndex].value === null || formItem.options[formItem.selectedIndex].value === "000") {
            ...
            bValidateOK = false;
            errorMessage = "Oops! The " + itemID + " is required.";
        }
        ...
        if (formItem.value.length < minLength) {
            ...
            // Length too long!
            bValidateOK = false;
            errorMessage = "Oops! The " + itemID + " has a maximum length of " + MaxLength + " characters.";
        }
            ...
            if (formItem.value.match(NotAllowedCharType) !== null) {
                ...
                switch (NotAllowedCharType) {
                    case lettersAndNums:
                        errorMessage = "Oops! The " + itemID + " can only be alpha-numeric characters.";
                        break;

                    case lettersOnly:
                        errorMessage = "Oops! The " + itemID + " can only be alphabetic characters.";
                        break;

                    case NumsOnly:
                        errorMessage = "Oops! The " + itemID + " can only be numeric characters.";
                        break;

                    default:
                        errorMessage = "Oops! The " + itemID + " can't use the type of characters you put in.";
                }// end switch chartype
            ...
            if (ReqCharTypes !== null && bValidateOK === true) {
                ...
                if (!bAllThere) {
                    ...
                    bValidateOK = false;
                    errorMessage = "Oops! The " + itemID + " doesn't look right.  Maybe you don't have everything you need?";
                }
